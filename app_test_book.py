...
def predict(image,parament):
    ...
    print(parament)
    with torch.no_grad():
            result = model.predict(img, conf=float(parament['conf']), imgsz=640, iou=0.45, save=False, device='0')
            list1 = []
            for r in result:
                ...
                if np.size(keypoints) !=0:
                    ...
                    keypoints = sort_keypoints(scaled_keypoints)

                    for points in keypoints:
                        if len(points) != 0:
                            points = points[0:4]
                            cropped1 = crop_rec(points, orig_img)

                            # 从排序后的列表中提取图像，并将它们添加到新的列表中
                            list2.append(cropped1)
...
def preprocess(image):
    received_array = pickle.loads(image)
