        ...
        for shelves in shelves_list:
            ...
            for book in book_list:
                if is_inside(book, shelves):
                    ...
                    book_dst_ = book_dst.reshape(-1, 2)

                    #norm_points = book_dst / np_w_h
                    norm_points_list = np.int32(np.round(book_dst_.tolist()))
                    if len(norm_points_list) == 4:
                        txt_content = " ".join(
                        [" ".join([str(cell[0]), str(cell[1])]) for cell in norm_points_list]) + " book " + "0\n"

                        cv2.polylines(img_per_, [norm_points_list], True, (0, 255, 0), 2)
