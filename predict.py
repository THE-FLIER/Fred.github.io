...
model = YOLO('models/best_book.pt')
...
for file_name in os.listdir(input):
    dir_list = [f"{output}/full", f"{output}/full_ori"]
    ...
    if file_name.endswith('.png') or file_name.endswith('.jpg'):
        ...
        h, w, _ = ori_img.shape
        results = model(source_path, conf=0.5, imgsz=640, save_txt=False, save_crop=False, boxes=False, device='0')  # results list
        a = file_name[:-4]
        filename = f'{a}.json'
        ...
        for r in results:
            ...
            for points in duplicated_rm:
                if len(points) != 0:
                    ...
                    # 保存结果
                    cv2.imwrite(f'{save}/{a}_{c}.jpg', cropped1)
